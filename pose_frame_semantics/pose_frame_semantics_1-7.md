...
The pose of a model is the location and orientation of the model in
a three-dimentional space. In SDFormat, we use a six-element vector
(position: x, y, z; orientation: roll, pitch, raw) to describe the pose.
Since pose is normally defined within a coordinate frame, every entity
(such as model, joint, link) in SDFormat defines the pose of an entity
with a [`//pose`](/tutorials?tut=specify_pose) tag. The coordinate frame
of which the pose relative to is specified using `//pose/@relative_to`
attribute.

In a nested hierarchy, it is also important to know how frames of each entity
relate to each other. SDFormat allows to establish this custom relation using
`//frame` tag and `//frame/@attached_to` attribute, which gives a frame name,
and defines which link or frame's movement the current frame is attached to.

Further syntax and rules regarding pose and frame are explained below.
